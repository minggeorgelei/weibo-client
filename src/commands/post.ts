...
export function registerPostCommands(program: Command, ctx: CliContext): void {
  program
    ...
    .option("--alt <text>", "Alt text for media (repeatable, matches --media order)", (val: string, acc: string[]) => {
      acc.push(val);
      return acc;
    }, [] as string[])
    .action(async (content: string, options: { media: string[]; alt: string[] }) => {
      ...
      let mediaSpecs;
      try {
        mediaSpecs = ctx.loadMedia({ media: options.media, alts: options.alt });
      } catch (err) {
        console.error(`${ctx.p("err")}${err instanceof Error ? err.message : String(err)}`);
        process.exit(1);
      }
      ...
      for (const spec of mediaSpecs) {
        if (spec.mime.startsWith("video/")) {
          ...
          if (!result.success) {
            console.error(`${ctx.p("err")}Failed to upload video: ${result.error}`);
            process.exit(1);
          }
          ...
          if (!result.success) {
            console.error(`${ctx.p("err")}Failed to upload image: ${result.error}`);
            process.exit(1);
          }
