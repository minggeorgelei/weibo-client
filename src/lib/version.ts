...
export function resolvePackageVersion(importMetaUrl?: string): string {
  const injected =
    ...
    typeof process.env.WEIBOCLIENT_VERSION === "string"
      ? process.env.WEIBOCLIENT_VERSION.trim()
      : "";
...
export function resolveGitSha(importMetaUrl?: string): string | null {
  const injected =
    typeof process !== "undefined" &&
    typeof process.env.WEIBOCLIENT_GIT_SHA === "string"
      ? process.env.WEIBOCLIENT_GIT_SHA.trim()
      : "";
  if (injected.length > 0) {
